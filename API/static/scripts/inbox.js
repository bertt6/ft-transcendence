...
class Inbox extends BaseComponent {
  ...
  render() {
    ...
    let buttonWrapper = this.parentElement.querySelector(
      ".inbox-element-interactions"
    );
    if (!buttonWrapper) {
      return;
    }
...
async function handleProfileImage() {
  ...
  document
    .getElementById("profile-image-wrapper")
    .setAttribute("href", `/profile/${profile.nickname}`);
}
...
export async function createInbox() {
  if (!getActiveUserNickname() && checkInboxRequired(window.location.pathname))
    return;
